...
data "aws_iam_role" "lab_role" {
  name = "LabRole"   # replace if name differs (probably not)
}
...
resource "aws_eks_cluster" "eks" {
  name     = "project-eks"
  role_arn = data.aws_iam_role.lab_role.arn   //aws_iam_role.master.arn
...
resource "aws_eks_node_group" "node-grp" {
  ...
  node_role_arn   = data.aws_iam_role.lab_role.arn  //aws_iam_role.worker.arn
  subnet_ids      = [data.aws_subnet.subnet-1.id, data.aws_subnet.subnet-2.id]
  capacity_type   = "ON_DEMAND"
  disk_size       = 20
  instance_types  = ["t3.medium"] // was t2.micro // was t2.larges
